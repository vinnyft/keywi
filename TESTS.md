...
Mot de passe de tous les comptes : `keywi123456`.
...
1. Navigateur A : <http://localhost:3000/connexion> → `hote1@keywi.fr`.
...
4. ✅ **Attendu** : bannière verte « Paiement confirmé », un **code badge**
   à 8 caractères s'affiche (ex. `KWIA1B2C`). Le notez — c'est lui qui
...
1. Sur la fiche de la clé, panneau **« Partager un accès »** : prénom
   `Léa`, email `voyageur@keywi.fr`, validité 7 jours → **Générer**.
...
1. Navigateur B : <http://localhost:3000/connexion> → `commerce3@keywi.fr`
   (Librairie du Marais — adaptez si vous avez choisi un autre commerce :
   `commerceN@keywi.fr` tient le N-ième point relais par ordre du seed).
...
1. (Optionnel) Navigateur privé : connexion `voyageur@keywi.fr`.
...
2. ✅ **Attendu** : l'app affiche **la case** (gros numéro), le logement et
   le bénéficiaire.
3. Test anti-erreur : re-scanner un **mauvais badge** (ex. `KWI7A2B4`,
...
docker exec supabase_db_keywi psql -U postgres -d postgres -c "
insert into keys (badge_uid, code_badge_imprime, logement, hote_id, relay_point_id, statut, paiement_statut)
values ('04:TEST:01','KWITEST1','Concurrence 1','20000000-0000-4000-a000-000000000001','a0000000-0000-4000-a000-000000000003','en_attente','offert'),
       ('04:TEST:02','KWITEST2','Concurrence 2','20000000-0000-4000-a000-000000000001','a0000000-0000-4000-a000-000000000003','en_attente','offert');"

for badge in KWITEST1 KWITEST2; do
  docker exec supabase_db_keywi psql -U postgres -d postgres -c "
  begin;
...
- Connecté `hote2@keywi.fr` : le tableau de bord ne montre **que** ses clés
  (jamais celles de `hote1`).
- Connecté `commerce1@keywi.fr` : `/commercant/cases` et l'historique ne
...
2. `admin@keywi.fr` → `/admin` : la candidature apparaît « à traiter » →
